...
void messageReceiver() {
  std::cout << "MESSAGE RECEIVER" << std::endl;
...
void processCommands() {
  while (gameRunning) {
    ...
    if (command_type == MOVE) {
      bool moveResult = game.move({msg[0] - '0', msg[1] - '0'},
...
void runGame() {
  ...
  while (gameRunning) {
    ...
    game.updateCurrCD(std::chrono::duration_cast<std::chrono::milliseconds>(
                          curr_time - prev_time)
                          .count());
    prev_time = curr_time;
    ...
    //            else if(readLen == 5){
    //                commandRead[4] = 0;
    //                sendMessage(&client_socket, MOVE, std::string(commandRead,
    //                5));
    //            }
    //            else if(readLen == 6){
    //                commandRead[4] -= '0';
